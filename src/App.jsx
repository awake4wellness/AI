...
function PatientDetailPlugin({ patient, sessions, onAddSession, navigate, plugins }) {
  ...
  async function save() { setSaving(true); await onAddSession({ ...form, paciente_id: patient.id, numero_sesion: patSess.length + 1 }); setShowModal(false); setSaving(false); } function abrirEdicion() { setEditForm({ nombre: patient.nombre || "", apellido: patient.apellido || "", edad: patient.edad || "", sexo: patient.sexo || "", telefono: patient.telefono || "", email: patient.email || "", condicion_principal: patient.condicion_principal || "", deporte: patient.deporte || "", nivel_actividad: patient.nivel_actividad || "Moderado", pais: patient.pais || "" }); setShowEdit(true); } async function guardarEdit() { setSavingEdit(true); const { error } = await CoreServices.update("patients", patient.id, editForm); setSavingEdit(false); if (error) { alert("No se pudo guardar: " + (error.message || "error")); return; } Object.assign(patient, editForm); setShowEdit(false); }
